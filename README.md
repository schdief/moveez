...
3. **Catalogue import:** use Add title to search OMDb (IMDb data), open a result to see poster, plot, IMDb rating, the Rotten Tomatoes audience score and the **moveez score** (IMDb × RT audience %, so 10 is perfect), jump to the IMDb/Rotten Tomatoes pages, and add it. Nothing else has to be entered by hand: after adding, the app looks up in the background
   ...
   - **FSK fallback** via Wikidata, and via the cinema programme for films in cinemas.
   - **Cinemas** (CinemaxX Dresden, Filmpalast Bautzen, UCI Dresden): `scripts/cinemas.mjs` reads the programmes from kinoprogramm.com in the Pages workflow (on every push and every 6 hours) and publishes them as `data/cinemas.json`; matching uses the original and the German TMDB title.

   The FSK filter hides titles without a known rating. On start-up the app refreshes the ratings of watchlist titles in the background (at most every 12 hours per title). The original Moveez OMDb key is built in; a personal key can be set in Settings. RT has no official API, so the audience score comes from the public search index the RT website itself uses and may break if RT changes it.
...
Rotten Tomatoes does not provide a supported public browser catalogue API, so the PWA does not scrape RT pages or claim live RT coverage. A future backend/provider integration can replace the catalogue adapter with licensed RT data. kinoprogramm.com has no official API either, so the cinema script may need adjustments if its HTML changes.
